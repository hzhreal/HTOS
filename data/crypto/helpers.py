...
from utils.constants import (
    ...
    DOA5_LR_TITLEID, RE6_TITLEID, MEARTH_SOM_TITLEID, MEARTH_SOW_TITLEID, GS3_TITLEID, JFORCE_TITLEID, BALATRO_TITLEID
)
...
async def extra_decrypt(
        ...
        async def decryption_callback(self, _: discord.Button, interaction: discord.Interaction) -> None:
            ...
            try:
                match self.game:
                    ...
                    case "JFORCE":
                        await Crypto.JForce.check_dec_ps(folderpath)
                    case "BALATRO":
    ...
    if title_id in GTAV_TITLEID:
        ...
        await d_ctx.msg.edit(embed=emb, view=CryptChoiceButton("CCR"))
        await helper.await_done()
        ...
        await d_ctx.msg.edit(embed=emb, view=CryptChoiceButton("TOB"))
        await helper.await_done()
        ...
        await d_ctx.msg.edit(embed=emb, view=CryptChoiceButton("TR6R"))
        await helper.await_done()
        ...
        await d_ctx.msg.edit(embed=emb, view=CryptChoiceButton("DIABLO3"))
        await helper.await_done()
    ...
    elif title_id in DOA5_LR_TITLEID:
        if choice is not None:
            if choice:
                try:
                    await Crypto.DoA5LR.check_dec_ps(folderpath)
    ...
    elif title_id in JFORCE_TITLEID:
        if choice is not None:
            if choice:
                try:
                    await Crypto.JForce.check_dec_ps(folderpath)
        ...
        await d_ctx.msg.edit(embed=emb, view=CryptChoiceButton("JFORCE"))
        await helper.await_done()

    elif title_id in BALATRO_TITLEID:
        if choice is not None:
            if choice:
                try:
                    await Crypto.Balatro.check_dec_ps(folderpath)
        ...
        await d_ctx.msg.edit(embed=emb, view=CryptChoiceButton("BALATRO"))
        await helper.await_done()
...
async def extra_import(title_id: str, folderpath: str, savepairname: str) -> None:
    try:
        if title_id in GTAV_TITLEID:
        ...
        elif title_id in SAO_FB_TITLEID:
            await Crypto.SAOFB.check_enc_ps(folderpath)

        elif title_id in MHR_TITLEID:
            await Crypto.MHR.check_enc_ps(folderpath)

        elif title_id in DOA5_LR_TITLEID:
            await Crypto.DoA5LR.check_enc_ps(folderpath)

        elif title_id in RE6_TITLEID:
            await Crypto.RE6.check_enc_ps(folderpath)
            ...
            await Crypto.JForce.check_enc_ps(folderpath)

        elif title_id in BALATRO_TITLEID:
            await Crypto.Balatro.check_enc_ps(folderpath)
    except (ValueError, IOError, IndexError):
        raise CryptoError("Invalid save!")
