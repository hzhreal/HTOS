...
class Crypt_FF7CC:
    KEY = b"ewKDrdIrMcAewKDrdE3SfRUL3SL0Fsa4"
    HEADER = b"kCAz"
    DEC_MAGIC = b"\x10\x00\x00\x00"
    ...
    async def decrypt_file(filepath: str) -> None:
        async with CC(filepath) as cc:
            await cc.r_stream.seek(0x55C)
            length = uint32(await cc.r_stream.read(4), "little").value
    ...
    async def encrypt_file(filepath: str) -> None:
        async with CC(filepath) as cc:
            ...
            while await cc.read(stop_off=0x568 + (length - 8)):
                ...
                # every multiple of 16 is a multiple of 8
                # therefore we can just trim the chunk, if it gets trimmed there is not anything else to encrypt either way
                if not cc._trim_chunk(8):
                ...
                for i in range(0, len(cc.chunk), 8):
                    ...
                    checksum = Crypt_FF7CC.C0[(r >> 0x18) & 0xFF] ^ Crypt_FF7CC.C1[(r >> 0x10) & 0xFF] ^ \
                               ...
                               Crypt_FF7CC.C4[(l >> 0x18) & 0xFF] ^ Crypt_FF7CC.C5[(l >> 0x10) & 0xFF] ^ \
                               Crypt_FF7CC.C6[(l >> 0x08) & 0xFF] ^ Crypt_FF7CC.C7[l & 0xFF]
                await cc.write()
            checksum = ~checksum
            checksum = ((checksum & 0xFF)       << 0x18) | ((checksum & 0xFF_00)       << 0x08) | \
                       ((checksum & 0xFF_00_00) >> 0x08) | ((checksum & 0xFF_00_00_00) >> 0x18)
            digest = uint32(checksum, "little").as_bytes
            await cc.ext_write(0x55C + 8, digest)
